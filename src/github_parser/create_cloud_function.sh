...
yc serverless function version create --function-name="$FUNCTION_NAME" --folder-id "$FOLDER_ID" --runtime python311 \
 --entrypoint parse_github_data.parse_github_data --memory 128m --execution-timeout 60s --source-path "$ZIP_PATH" \
...
if [ -n "$EXISTING_TRIGGER" ]; then
    ...
    yc serverless trigger create timer --name "$TRIGGER_NAME" --cron-expression '*/30 * * * ? *' --invoke-function-id "$FUNCTION_ID" --folder-id "$FOLDER_ID" --invoke-function-service-account-id "$SERVICE_ACCOUNT_ID"
fi